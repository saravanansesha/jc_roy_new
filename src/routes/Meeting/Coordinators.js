...
// import BreadCrumb from "../../coordinatorss/BreadCrumbs"
// import Setproductadd from "./Setproductadd"
import Add_coordinators from "./Add_coordinators"
// const DesignStandard = Tabs.TabPane;
const {Option, OptGroup} = Select;
const FormItem = Form.Item;
const Coordinators = ({match}) => {
  ...
  const columns = [
    {
      ...
      if(check_presence.length > 0){
        ...
        values[`coordinators${index}`] = value1[0].coordinators
        ...
        function onFinish(values){
          let datason = data_back.map((item)=>{
            if(item.key == find_key){
              ...
              forchange.coordinators = values[`coordinators${index}`]
        ...
        function onFinishFailed(){
          ...
          layout="inline">
                     <FormItem label={`Title`} rules={[{ required: true, message: 'Please input your Title!\'}' }]} name={`coordinators${index}`}>
...
<Add_coordinators setvisible={setvisible} isvisible={isvisible} />
