...
// import BreadCrumb from "../../venuess/BreadCrumbs"
// import Setproductadd from "./Setproductadd"
import Add_venues from "./Add_venues"
  ...
  const columns = [
    {
      ...
      if(check_presence.length > 0){
        ...
        values[`venues${index}`] = value1[0].venues
        ...
        function onFinish(values){
          let datason = data_back.map((item)=>{
            if(item.key == find_key){
              ...
              forchange.venues = values[`venues${index}`]
        ...
        function onFinishFailed(){
          ...
          layout="inline">
                     <FormItem label={`Title`} rules={[{ required: true, message: 'Please input your Title!\'}' }]} name={`venues${index}`}>
...
<Add_venues setvisible={setvisible} isvisible={isvisible} />
