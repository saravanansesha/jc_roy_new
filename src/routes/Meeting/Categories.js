...
// import BreadCrumb from "../../categoriess/BreadCrumbs"
  ...
  const columns = [
    {
      ...
      if(check_presence.length > 0){
        ...
        values[`categories${index}`] = value1[0].categories
        ...
        function onFinish(values){
          let datason = data_back.map((item)=>{
            if(item.key == find_key){
              ...
              forchange.categories = values[`categories${index}`]
        ...
        function onFinishFailed(){
          ...
          layout="inline">
                     <FormItem label={`Title`} rules={[{ required: true, message: 'Please input your Title!\'}' }]} name={`categories${index}`}>
...
<Add_categories setvisible={setvisible} isvisible={isvisible} />
