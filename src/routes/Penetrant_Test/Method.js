...
// import BreadCrumb from "../../methods/BreadCrumbs"
// import Setproductadd from "./Setproductadd"
import Add_method from "./Add_method"
  ...
  const columns = [
    {
      ...
      if(check_presence.length > 0){
        ...
        values[`method${index}`] = value1[0].method
        ...
        function onFinish(values){
          let datason = data_back.map((item)=>{
            if(item.key == find_key){
              ...
              forchange.method = values[`method${index}`]
        ...
        function onFinishFailed(){
          ...
          layout="inline">
                     <FormItem label={`Title`} rules={[{ required: true, message: 'Please input your Title!\'}' }]} name={`method${index}`}>
...
<Add_method setvisible={setvisible} isvisible={isvisible} />
