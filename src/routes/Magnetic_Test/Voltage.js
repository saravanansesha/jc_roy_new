...
// import BreadCrumb from "../../voltages/BreadCrumbs"
// import Setproductadd from "./Setproductadd"
import Add_voltage from "./Add_voltage"
  ...
  const columns = [
    {
      ...
      if(check_presence.length > 0){
        ...
        values[`voltage${index}`] = value1[0].voltage
        ...
        function onFinish(values){
          let datason = data_back.map((item)=>{
            if(item.key == find_key){
              ...
              forchange.voltage = values[`voltage${index}`]
        ...
        function onFinishFailed(){
          ...
          layout="inline">
                     <FormItem label={`Title`} rules={[{ required: true, message: 'Please input your Title!\'}' }]} name={`voltage${index}`}>
...
<Add_voltage setvisible={setvisible} isvisible={isvisible} />
