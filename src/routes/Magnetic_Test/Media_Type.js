...
// import BreadCrumb from "../../media_Types/BreadCrumbs"
// import Setproductadd from "./Setproductadd"
import Add_media_Type from "./Add_media_Type"
// const DesignStandard = Tabs.TabPane;
const {Option, OptGroup} = Select;
const FormItem = Form.Item;
const Media_Type = ({match}) => {
  ...
  const columns = [
    {
      ...
      if(check_presence.length > 0){
        ...
        values[`media_Type${index}`] = value1[0].media_Type
        ...
        function onFinish(values){
          let datason = data_back.map((item)=>{
            if(item.key == find_key){
              ...
              forchange.media_Type = values[`media_Type${index}`]
        ...
        function onFinishFailed(){
          ...
          layout="inline">
                     <FormItem label={`Title`} rules={[{ required: true, message: 'Please input your Title!\'}' }]} name={`media_Type${index}`}>
...
<Add_media_Type setvisible={setvisible} isvisible={isvisible} />
