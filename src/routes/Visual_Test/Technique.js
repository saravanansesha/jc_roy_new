...
// import BreadCrumb from "../../techniques/BreadCrumbs"
// import Setproductadd from "./Setproductadd"
import Add_technique from "./Add_technique"
  ...
  const columns = [
    {
      ...
      if(check_presence.length > 0){
        ...
        values[`technique${index}`] = value1[0].technique
        ...
        function onFinish(values){
          let datason = data_back.map((item)=>{
            if(item.key == find_key){
              ...
              forchange.technique = values[`technique${index}`]
        ...
        function onFinishFailed(){
          ...
          layout="inline">
                     <FormItem label={`Title`} rules={[{ required: true, message: 'Please input your Title!\'}' }]} name={`technique${index}`}>
...
<Add_technique setvisible={setvisible} isvisible={isvisible} />
