...
// import BreadCrumb from "../../transfer_Corrections/BreadCrumbs"
// import Setproductadd from "./Setproductadd"
import Add_transfer_Correction from "./Add_transfer_Correction"
// const DesignStandard = Tabs.TabPane;
const {Option, OptGroup} = Select;
const FormItem = Form.Item;
const Transfer_Correction = ({match}) => {
  ...
  const [data_back,setdata] = useState([
    {
      key: '1',
      transfer_Correction: 'transfer_Correctionss1',
      status: 'inactive',
    ...
    {
      key: '2',
      transfer_Correction: 'transfer_Correctionss2',
      status: 'active',
      permissions : 'permissions',
      action : <div>
      <button>
    ...
    },
    {
      key: '3',
      status: 'active',
      transfer_Correction: 'transfer_Correctionss3',
      permissions : 'permissions',
      action : <div>
        <button>
            Edit
    ...
    },
    {
      key: '4',
      status: 'active',
      transfer_Correction: 'transfer_Correctionss4',
      permissions : 'permissions',
      action : <div>
        <button>
            Edit
    ...
    {
      key: '5',
      transfer_Correction: 'transfer_Correctionss5',
      status: 'active',
      permissions : 'permissions',
      action : <div>
        <button>
    ...
    {
      key: '6',
      transfer_Correction: 'transfer_Correctionss6',
      status: 'active',
      permissions : 'permissions',
      action : <div>
        <button>
  ...
  const columns = [
    {
      title: 'Title',
      dataIndex: 'transfer_Correction',
      key: 'transfer_Correction',
      ...
      if(check_presence.length > 0){
        ...
        values[`transfer_Correction${index}`] = value1[0].transfer_Correction
        ...
        function onFinish(values){
          let datason = data_back.map((item)=>{
            if(item.key == find_key){
              ...
              forchange.transfer_Correction = values[`transfer_Correction${index}`]
        ...
        function onFinishFailed(){
          ...
          layout="inline">
                     <FormItem label={`Title`} rules={[{ required: true, message: 'Please input your Title!\'}' }]} name={`transfer_Correction${index}`}>
    ...
    let check_presence =   foredit.filter((items)=>{    
      ...
      if(check_presence.length > 0){
        ...
        return {
          ...
          key : item.key,
          transfer_Correction : item.transfer_Correction,
          created : '06-09-2022',
          modified : '07-09-2022',
...
<Add_transfer_Correction setvisible={setvisible} isvisible={isvisible} />
{/* <Setproductadd setvisible={setvisible} isvisible={isvisible} /> */}
    </div>
  );
};
    
export default Transfer_Correction;
