...
// import BreadCrumb from "../../scan_Scopes/BreadCrumbs"
// import Setproductadd from "./Setproductadd"
import Add_scan_Scope from "./Add_scan_Scope"
// const DesignStandard = Tabs.TabPane;
const {Option, OptGroup} = Select;
const FormItem = Form.Item;
const Scan_Scope = ({match}) => {
  ...
  const columns = [
    {
      ...
      if(check_presence.length > 0){
        ...
        values[`scan_Scope${index}`] = value1[0].scan_Scope
        ...
        function onFinish(values){
          let datason = data_back.map((item)=>{
            if(item.key == find_key){
              ...
              forchange.scan_Scope = values[`scan_Scope${index}`]
        ...
        function onFinishFailed(){
          ...
          layout="inline">
                     <FormItem label={`Title`} rules={[{ required: true, message: 'Please input your Title!\'}' }]} name={`scan_Scope${index}`}>
...
<Add_scan_Scope setvisible={setvisible} isvisible={isvisible} />
{/* <Setproductadd setvisible={setvisible} isvisible={isvisible} /> */}
    </div>
  );
};
    
export default Scan_Scope;
