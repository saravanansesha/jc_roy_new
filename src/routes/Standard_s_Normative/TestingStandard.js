...
import Add_testing from "./Add_design"
  ...
  const columns = [
    {
      ...
      if(check_presence.length > 0){
        ...
        values[`testing_standard${index}`] = value1[0].testing_standard
        ...
        function onFinish(values){
          let datason = data_back.map((item)=>{
            if(item.key == find_key){
              ...
              forchange.testing_standard = values[`testing_standard${index}`]
        ...
        function onFinishFailed(){
          ...
          layout="inline">
                     <FormItem label={`Testing Standard`} rules={[{ required: true, message: 'Please input your Testing Standard!\'}' }]} name={`testing_standard${index}`}>
...
<Add_testing setvisible={setvisible} isvisible={isvisible} />
