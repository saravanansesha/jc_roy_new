...
import Add_design from "./Add_design"
// const DesignStandard = Tabs.TabPane;
const {Option, OptGroup} = Select;
const FormItem = Form.Item;
const DesignStandard = ({match}) => {
  ...
  const columns = [
    {
      ...
      if(check_presence.length > 0){
        ...
        values[`design_standard${index}`] = value1[0].design_standard
        ...
        function onFinish(values){
          let datason = data_back.map((item)=>{
            if(item.key == find_key){
              ...
              forchange.design_standard = values[`design_standard${index}`]
        ...
        function onFinishFailed(){
          ...
          layout="inline">
                     <FormItem label={`Design Standard`} rules={[{ required: true, message: 'Please input your Design Standard!\'}' }]} name={`design_standard${index}`}>

<Input placeholder="Design Standard"/>
</FormItem>
    ...
    let check_presence =   foredit.filter((items)=>{    
      ...
      if(check_presence.length > 0){
        ...
        return {
          ...
          key : item.key,
          design_standard : item.design_standard,
          created : '06-09-2022',
          modified : '07-09-2022',
...
<Add_design setvisible={setvisible} isvisible={isvisible} />
