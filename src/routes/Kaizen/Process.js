...
// import BreadCrumb from "../../processs/BreadCrumbs"
  ...
  const columns = [
    {
      ...
      if(check_presence.length > 0){
        ...
        values[`process${index}`] = value1[0].process
        ...
        function onFinish(values){
          let datason = data_back.map((item)=>{
            if(item.key == find_key){
              ...
              forchange.process = values[`process${index}`]
        ...
        function onFinishFailed(){
          ...
          layout="inline">
                     <FormItem label={`Title`} rules={[{ required: true, message: 'Please input your Title!\'}' }]} name={`process${index}`}>
...
<Add_process setvisible={setvisible} isvisible={isvisible} />
