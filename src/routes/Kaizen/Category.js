...
// import BreadCrumb from "../../categorys/BreadCrumbs"
  ...
  const columns = [
    {
      ...
      if(check_presence.length > 0){
        ...
        values[`category${index}`] = value1[0].category
        ...
        function onFinish(values){
          let datason = data_back.map((item)=>{
            if(item.key == find_key){
              ...
              forchange.category = values[`category${index}`]
        ...
        function onFinishFailed(){
          ...
          layout="inline">
                     <FormItem label={`Title`} rules={[{ required: true, message: 'Please input your Title!\'}' }]} name={`category${index}`}>
...
<Add_category setvisible={setvisible} isvisible={isvisible} />
