...
// import BreadCrumb from "../../focus_Areas/BreadCrumbs"
// import Setproductadd from "./Setproductadd"
import Add_focus_Area from "./Add_focus_Area"
// const DesignStandard = Tabs.TabPane;
const {Option, OptGroup} = Select;
const FormItem = Form.Item;
const Focus_Area = ({match}) => {
  ...
  const columns = [
    {
      ...
      if(check_presence.length > 0){
        ...
        values[`focus_Area${index}`] = value1[0].focus_Area
        ...
        function onFinish(values){
          let datason = data_back.map((item)=>{
            if(item.key == find_key){
              ...
              forchange.focus_Area = values[`focus_Area${index}`]
        ...
        function onFinishFailed(){
          ...
          layout="inline">
                     <FormItem label={`Title`} rules={[{ required: true, message: 'Please input your Title!\'}' }]} name={`focus_Area${index}`}>
...
<Add_focus_Area setvisible={setvisible} isvisible={isvisible} />
