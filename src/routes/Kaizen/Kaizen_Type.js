...
// import BreadCrumb from "../../kaizen_Types/BreadCrumbs"
// import Setproductadd from "./Setproductadd"
import Add_kaizen_Type from "./Add_kaizen_Type"
// const DesignStandard = Tabs.TabPane;
const {Option, OptGroup} = Select;
const FormItem = Form.Item;
const Kaizen_Type = ({match}) => {
  ...
  const columns = [
    {
      ...
      if(check_presence.length > 0){
        ...
        values[`kaizen_Type${index}`] = value1[0].kaizen_Type
        ...
        function onFinish(values){
          let datason = data_back.map((item)=>{
            if(item.key == find_key){
              ...
              forchange.kaizen_Type = values[`kaizen_Type${index}`]
        ...
        function onFinishFailed(){
          ...
          layout="inline">
                     <FormItem label={`Title`} rules={[{ required: true, message: 'Please input your Title!\'}' }]} name={`kaizen_Type${index}`}>
...
<Add_kaizen_Type setvisible={setvisible} isvisible={isvisible} />
{/* <Setproductadd setvisible={setvisible} isvisible={isvisible} /> */}
    </div>
  );
};
    
export default Kaizen_Type;