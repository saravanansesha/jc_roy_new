...
// import BreadCrumb from "../../audit_Areas/BreadCrumbs"
// import Setproductadd from "./Setproductadd"
import Add_audit_Area from "./Add_audit_Area"
// const DesignStandard = Tabs.TabPane;
const {Option, OptGroup} = Select;
const FormItem = Form.Item;
const Audit_Area = ({match}) => {
  ...
  const columns = [
    {
      ...
      if(check_presence.length > 0){
        ...
        values[`audit_Area${index}`] = value1[0].audit_Area
        ...
        function onFinish(values){
          let datason = data_back.map((item)=>{
            if(item.key == find_key){
              ...
              forchange.audit_Area = values[`audit_Area${index}`]
        ...
        function onFinishFailed(){
          ...
          layout="inline">
                     <FormItem label={`Title`} rules={[{ required: true, message: 'Please input your Title!\'}' }]} name={`audit_Area${index}`}>
...
<Add_audit_Area setvisible={setvisible} isvisible={isvisible} />
{/* <Setproductadd setvisible={setvisible} isvisible={isvisible} /> */}
    </div>
  );
};
    
export default Audit_Area;
