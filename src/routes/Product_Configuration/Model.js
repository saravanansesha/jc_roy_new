...
import Add_model from "./Add_model"
  ...
  const columns = [
    {
      ...
      if(check_presence.length > 0){
        ...
        values[`modelname${index}`] = value1[0].modelname
        ...
        function onFinish(values){
          let datason = data_back.map((item)=>{
            if(item.key == find_key){
              ...
              forchange.modelname = values[`modelname${index}`]
        ...
        function onFinishFailed(){
          ...
          layout="inline">
                     <FormItem label={`Model Name`} rules={[{ required: true, message: 'Please input your Model Name!\'}' }]} name={`modelname${index}`}>
...
<Add_model setvisible={setvisible} isvisible={isvisible} />
