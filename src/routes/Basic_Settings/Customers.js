...
import Edit_customername from "./Edit_customername"
import Add_customername from "./Add_customername"
    ...
    let check_presence =   foredit.filter((items)=>{    
      ...
      if(check_presence.length > 0){
        ...
        return {
            ...
            contact: <><span>{item.contact.primary_contact_person}</span><br></br><span>{item.contact.secondary_contact_person}</span></>,
  ...
  function editinformation(values,key){
      let updatepack = data_back.map((item)=>{
        if(item.key == key){
            let full_date;
            let evfull_date;
            if('expiry_date_of_approval' in values){
                let date = new Date(values.expiry_date_of_approval)
                let mnth = ("0" + (date.getMonth() + 1)).slice(-2)
                let day = ("0" + date.getDate()).slice(-2)
                 full_date = [date.getFullYear(), mnth, day].join("-")
            }
            if('intitial_evaluation_date' in values){
                ...
                evfull_date = [evdate.getFullYear(), evmnth, evday].join("-")
            }
          return     {
            key: item.key,
            firstname:  'firstname' in values ? values.firstname :'',
            lastname:  'lastname' in values ? values.lastname : '',
            customername:  'customername' in values ? values.customername : '',
            id: 'id' in values ? values.id :'',
            plant:  'plant' in values ? values.plant :'',
            depart: 'depart' in values ? values.depart :'',
            role: 'role' in values ? values.role :'',
            plant_head:  'plant_head' in values ? values.plant_head :'',
            dep_head:  'dep_head' in values ? values.dep_head :'',
            manage_rep:  'manage_rep' in values ? values.manage_rep :'',
            ccr:  'ccr' in values ? values.ccr :'',
            email:  'email' in values ? values.email :'',
            profile:  'profile' in values ? values.profile :'',
            status:  'status' in values ? values.status :'',
            password:  'password' in values ? values.password :'',
            permissions :  'permissions' in values ? item.permissions :'',
            customername: 'customername' in values ? values.customername :'',
      address: 'address' in values ? values.address :'',
      country_of_origin: 'country_of_origin' in values ? values.country_of_origin :'',
      email : 'country_of_origin' in values ? values.country_of_origin :'',
      parent_company: 'parent_company' in values ? values.parent_company :'',
      contact :{
        primary_contact_person: 'primary_contact_person' in values ? values.primary_contact_person :'',
        secondary_contact_person: 'secondary_contact_person' in values ? values.secondary_contact_person :'',
      },
      classification_type: 'classification_type' in values ? values.classification_type :'',
      product_category: 'product_category' in values ? values.product_category :'',
      scope_of_supply: 'scope_of_supply' in values ? values.scope_of_supply :'',
      type_of_approval: 'type_of_approval' in values ? values.type_of_approval :'',
       expiry_date_of_approval: 'expiry_date_of_approval'in values ? full_date :'',
       intitial_evaluation_date: 'intitial_evaluation_date' in values ? evfull_date :'',
      title: 'title' in values ? values.title :'',
      status_of_approval : 'status_of_approval ' in values ? values.status_of_approval  :'',
      remarks_observations : 'remarks_observations ' in values ? values.remarks_observations  :'',
      contacts: 'contacts' in values ? values.contacts :'',
      product: 'product' in values ? values.product :'',
            action : <div>
              <button>
                  Edit
              </button>
...
<Edit_customername editinformation={editinformation} pop_upedit={pop_upedit} setvisible={setvisible} isvisible={isvisible} />
<Add_customername setaddvisible={setaddvisible} addisvisible={addisvisible} />
