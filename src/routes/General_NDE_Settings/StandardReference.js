...
// import BreadCrumb from "../../StandardReferences/BreadCrumbs"
// import Setproductadd from "./Setproductadd"
import Add_StandardReference from "./Add_StandardReference"
// const DesignStandard = Tabs.TabPane;
const {Option, OptGroup} = Select;
const FormItem = Form.Item;
const StandardReference = ({match}) => {
  ...
  const columns = [
    {
      ...
      if(check_presence.length > 0){
        ...
        values[`StandardReference${index}`] = value1[0].StandardReference
        ...
        function onFinish(values){
          let datason = data_back.map((item)=>{
            if(item.key == find_key){
              ...
              forchange.StandardReference = values[`StandardReference${index}`]
        ...
        function onFinishFailed(){
          ...
          layout="inline">
                     <FormItem label={`Title`} rules={[{ required: true, message: 'Please input your Title!\'}' }]} name={`StandardReference${index}`}>
...
<Add_StandardReference setvisible={setvisible} isvisible={isvisible} />
