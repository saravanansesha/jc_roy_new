...
// import BreadCrumb from "../../operators/BreadCrumbs"
// import Setproductadd from "./Setproductadd"
import Add_operator from "./Add_operator"
  ...
  const columns = [
    {
      ...
      if(check_presence.length > 0){
        ...
        values[`operator${index}`] = value1[0].operator
        ...
        function onFinish(values){
          let datason = data_back.map((item)=>{
            if(item.key == find_key){
              ...
              forchange.operator = values[`operator${index}`]
        ...
        function onFinishFailed(){
          ...
          layout="inline">
                     <FormItem label={`Title`} rules={[{ required: true, message: 'Please input your Title!\'}' }]} name={`operator${index}`}>
...
<Add_operator setvisible={setvisible} isvisible={isvisible} />
