...
// import BreadCrumb from "../../test_stages/BreadCrumbs"
// import Setproductadd from "./Setproductadd"
import Add_Test_Stage from "./Add_Test_Stage"
// const DesignStandard = Tabs.TabPane;
const {Option, OptGroup} = Select;
const FormItem = Form.Item;
const Test_Stage = ({match}) => {
  ...
  const columns = [
    {
      ...
      if(check_presence.length > 0){
        ...
        values[`test_stage${index}`] = value1[0].test_stage
        ...
        function onFinish(values){
          let datason = data_back.map((item)=>{
            if(item.key == find_key){
              ...
              forchange.test_stage = values[`test_stage${index}`]
        ...
        function onFinishFailed(){
          ...
          layout="inline">
                     <FormItem label={`Title`} rules={[{ required: true, message: 'Please input your Title!\'}' }]} name={`test_stage${index}`}>
...
<Add_Test_Stage setvisible={setvisible} isvisible={isvisible} />
{/* <Setproductadd setvisible={setvisible} isvisible={isvisible} /> */}
    </div>
  );
};
    
export default Test_Stage;
