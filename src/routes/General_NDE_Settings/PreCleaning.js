...
// import BreadCrumb from "../../preCleanings/BreadCrumbs"
// import Setproductadd from "./Setproductadd"
import Add_preCleaning from "./Add_preCleaning"
// const DesignStandard = Tabs.TabPane;
const {Option, OptGroup} = Select;
const FormItem = Form.Item;
const PreCleaning = ({match}) => {
  ...
  const columns = [
    {
      ...
      if(check_presence.length > 0){
        ...
        values[`preCleaning${index}`] = value1[0].preCleaning
        ...
        function onFinish(values){
          let datason = data_back.map((item)=>{
            if(item.key == find_key){
              ...
              forchange.preCleaning = values[`preCleaning${index}`]
        ...
        function onFinishFailed(){
          ...
          layout="inline">
                     <FormItem label={`Title`} rules={[{ required: true, message: 'Please input your Title!\'}' }]} name={`preCleaning${index}`}>
...
<Add_preCleaning setvisible={setvisible} isvisible={isvisible} />
