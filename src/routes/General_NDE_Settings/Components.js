...
import Add_component from "./Add_component"
  ...
  const columns = [
    {
      ...
      if(check_presence.length > 0){
        ...
        values[`component${index}`] = value1[0].component
        ...
        function onFinish(values){
          let datason = data_back.map((item)=>{
            if(item.key == find_key){
              ...
              forchange.component = values[`component${index}`]
        ...
        function onFinishFailed(){
          ...
          layout="inline">
                     <FormItem label={`Title`} rules={[{ required: true, message: 'Please input your Title!\'}' }]} name={`component${index}`}>
...
<Add_component setvisible={setvisible} isvisible={isvisible} />
