...
// import BreadCrumb from "../../ncrProcesss/BreadCrumbs"
// import Setproductadd from "./Setproductadd"
import Add_ncrProcess from "./Add_ncrProcess"
// const DesignStandard = Tabs.TabPane;
const {Option, OptGroup} = Select;
const FormItem = Form.Item;
const NcrProcess = ({match}) => {
  ...
  const columns = [
    {
      ...
      if(check_presence.length > 0){
        ...
        values[`ncrProcess${index}`] = value1[0].ncrProcess
        ...
        function onFinish(values){
          let datason = data_back.map((item)=>{
            if(item.key == find_key){
              ...
              forchange.ncrProcess = values[`ncrProcess${index}`]
        ...
        function onFinishFailed(){
          ...
          layout="inline">
                     <FormItem label={`Title`} rules={[{ required: true, message: 'Please input your Title!\'}' }]} name={`ncrProcess${index}`}>
...
<Add_ncrProcess setvisible={setvisible} isvisible={isvisible} />
{/* <Setproductadd setvisible={setvisible} isvisible={isvisible} /> */}
    </div>
  );
};
    
export default NcrProcess;
