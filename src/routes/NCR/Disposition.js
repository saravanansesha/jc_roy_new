...
// import BreadCrumb from "../../dispositions/BreadCrumbs"
// import Setproductadd from "./Setproductadd"
import Add_Disposition from "./Add_Disposition"
// const DesignStandard = Tabs.TabPane;
const {Option, OptGroup} = Select;
const FormItem = Form.Item;
const Disposition = ({match}) => {
  ...
  const columns = [
    {
      ...
      if(check_presence.length > 0){
        ...
        values[`disposition${index}`] = value1[0].disposition
        ...
        function onFinish(values){
          let datason = data_back.map((item)=>{
            if(item.key == find_key){
              ...
              forchange.disposition = values[`disposition${index}`]
        ...
        function onFinishFailed(){
          ...
          layout="inline">
                     <FormItem label={`Title`} rules={[{ required: true, message: 'Please input your Title!\'}' }]} name={`disposition${index}`}>
...
<Add_Disposition setvisible={setvisible} isvisible={isvisible} />
