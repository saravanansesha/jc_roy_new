...
// import BreadCrumb from "../../ncrtypes/BreadCrumbs"
// import Setproductadd from "./Setproductadd"
import Add_ncrtype from "./Add_ncrtype"
  ...
  const columns = [
    {
      ...
      if(check_presence.length > 0){
        ...
        values[`ncrtype${index}`] = value1[0].ncrtype
        ...
        function onFinish(values){
          let datason = data_back.map((item)=>{
            if(item.key == find_key){
              ...
              forchange.ncrtype = values[`ncrtype${index}`]
        ...
        function onFinishFailed(){
          ...
          layout="inline">
                     <FormItem label={`Title`} rules={[{ required: true, message: 'Please input your Title!\'}' }]} name={`ncrtype${index}`}>
...
<Add_ncrtype setvisible={setvisible} isvisible={isvisible} />
{/* <Setproductadd setvisible={setvisible} isvisible={isvisible} /> */}
    </div>
  );
};
    
export default Ncrtype;
