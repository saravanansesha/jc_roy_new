...
// import BreadCrumb from "../../impact_Areas/BreadCrumbs"
// import Setproductadd from "./Setproductadd"
import Add_impact_Area from "./Add_impact_Area"
// const DesignStandard = Tabs.TabPane;
const {Option, OptGroup} = Select;
const FormItem = Form.Item;
const Impact_Area = ({match}) => {
  ...
  const columns = [
    {
      ...
      if(check_presence.length > 0){
        ...
        values[`impact_Area${index}`] = value1[0].impact_Area
        ...
        function onFinish(values){
          let datason = data_back.map((item)=>{
            if(item.key == find_key){
              ...
              forchange.impact_Area = values[`impact_Area${index}`]
        ...
        function onFinishFailed(){
          ...
          layout="inline">
                     <FormItem label={`Title`} rules={[{ required: true, message: 'Please input your Title!\'}' }]} name={`impact_Area${index}`}>
...
<Add_impact_Area setvisible={setvisible} isvisible={isvisible} />
{/* <Setproductadd setvisible={setvisible} isvisible={isvisible} /> */}
    </div>
  );
};
    
export default Impact_Area;
