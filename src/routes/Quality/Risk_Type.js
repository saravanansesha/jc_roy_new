...
// import BreadCrumb from "../../risk_Types/BreadCrumbs"
// import Setproductadd from "./Setproductadd"
import Add_risk_Type from "./Add_risk_Type"
// const DesignStandard = Tabs.TabPane;
const {Option, OptGroup} = Select;
const FormItem = Form.Item;
const Risk_Type = ({match}) => {
  ...
  const columns = [
    {
      ...
      if(check_presence.length > 0){
        ...
        values[`risk_Type${index}`] = value1[0].risk_Type
        ...
        function onFinish(values){
          let datason = data_back.map((item)=>{
            if(item.key == find_key){
              ...
              forchange.risk_Type = values[`risk_Type${index}`]
        ...
        function onFinishFailed(){
          ...
          layout="inline">
                     <FormItem label={`Title`} rules={[{ required: true, message: 'Please input your Title!\'}' }]} name={`risk_Type${index}`}>
...
<Add_risk_Type setvisible={setvisible} isvisible={isvisible} />
{/* <Setproductadd setvisible={setvisible} isvisible={isvisible} /> */}
    </div>
  );
};
    
export default Risk_Type;
