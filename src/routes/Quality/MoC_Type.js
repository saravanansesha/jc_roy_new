...
// import BreadCrumb from "../../MoC_Types/BreadCrumbs"
// import Setproductadd from "./Setproductadd"
import Add_MoC_Type from "./Add_MoC_Type"
// const DesignStandard = Tabs.TabPane;
const {Option, OptGroup} = Select;
const FormItem = Form.Item;
const MoC_Type = ({match}) => {
  ...
  const columns = [
    {
      ...
      if(check_presence.length > 0){
        ...
        values[`MoC_Type${index}`] = value1[0].MoC_Type
        ...
        function onFinish(values){
          let datason = data_back.map((item)=>{
            if(item.key == find_key){
              ...
              forchange.MoC_Type = values[`MoC_Type${index}`]
        ...
        function onFinishFailed(){
          ...
          layout="inline">
                     <FormItem label={`Title`} rules={[{ required: true, message: 'Please input your Title!\'}' }]} name={`MoC_Type${index}`}>
...
<Add_MoC_Type setvisible={setvisible} isvisible={isvisible} />
{/* <Setproductadd setvisible={setvisible} isvisible={isvisible} /> */}
    </div>
  );
};
    
export default MoC_Type;
