...
// import BreadCrumb from "../../risk_Sources/BreadCrumbs"
// import Setproductadd from "./Setproductadd"
import Add_risk_Source from "./Add_risk_Source"
// const DesignStandard = Tabs.TabPane;
const {Option, OptGroup} = Select;
const FormItem = Form.Item;
const Risk_Source = ({match}) => {
  ...
  const columns = [
    {
      ...
      if(check_presence.length > 0){
        ...
        values[`risk_Source${index}`] = value1[0].risk_Source
        ...
        function onFinish(values){
          let datason = data_back.map((item)=>{
            if(item.key == find_key){
              ...
              forchange.risk_Source = values[`risk_Source${index}`]
        ...
        function onFinishFailed(){
          ...
          layout="inline">
                     <FormItem label={`Title`} rules={[{ required: true, message: 'Please input your Title!\'}' }]} name={`risk_Source${index}`}>
...
<Add_risk_Source setvisible={setvisible} isvisible={isvisible} />
{/* <Setproductadd setvisible={setvisible} isvisible={isvisible} /> */}
    </div>
  );
};
    
export default Risk_Source;
