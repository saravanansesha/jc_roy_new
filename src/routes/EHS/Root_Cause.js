...
// import BreadCrumb from "../../root_Causes/BreadCrumbs"
// import Setproductadd from "./Setproductadd"
import Add_root_Cause from "./Add_root_Cause"
// const DesignStandard = Tabs.TabPane;
const {Option, OptGroup} = Select;
const FormItem = Form.Item;
const Root_Cause = ({match}) => {
  ...
  const columns = [
    {
      ...
      if(check_presence.length > 0){
        ...
        values[`root_Cause${index}`] = value1[0].root_Cause
        ...
        function onFinish(values){
          let datason = data_back.map((item)=>{
            if(item.key == find_key){
              ...
              forchange.root_Cause = values[`root_Cause${index}`]
        ...
        function onFinishFailed(){
          ...
          layout="inline">
                     <FormItem label={`Title`} rules={[{ required: true, message: 'Please input your Title!\'}' }]} name={`root_Cause${index}`}>
...
<Add_root_Cause setvisible={setvisible} isvisible={isvisible} />
{/* <Setproductadd setvisible={setvisible} isvisible={isvisible} /> */}
    </div>
  );
};
    
export default Root_Cause;
