...
// import BreadCrumb from "../../main_Causes/BreadCrumbs"
// import Setproductadd from "./Setproductadd"
import Add_main_Cause from "./Add_main_Cause"
// const DesignStandard = Tabs.TabPane;
const {Option, OptGroup} = Select;
const FormItem = Form.Item;
const Main_Cause = ({match}) => {
  ...
  const columns = [
    {
      ...
      if(check_presence.length > 0){
        ...
        values[`main_Cause${index}`] = value1[0].main_Cause
        ...
        function onFinish(values){
          let datason = data_back.map((item)=>{
            if(item.key == find_key){
              ...
              forchange.main_Cause = values[`main_Cause${index}`]
        ...
        function onFinishFailed(){
          ...
          layout="inline">
                     <FormItem label={`Title`} rules={[{ required: true, message: 'Please input your Title!\'}' }]} name={`main_Cause${index}`}>
...
<Add_main_Cause setvisible={setvisible} isvisible={isvisible} />
{/* <Setproductadd setvisible={setvisible} isvisible={isvisible} /> */}
    </div>
  );
};
    
export default Main_Cause;
