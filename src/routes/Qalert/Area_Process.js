...
// import BreadCrumb from "../../area_Processs/BreadCrumbs"
// import Setproductadd from "./Setproductadd"
import Add_area_Process from "./Add_area_Process"
// const DesignStandard = Tabs.TabPane;
const {Option, OptGroup} = Select;
const FormItem = Form.Item;
const Area_area_Process = ({match}) => {
  ...
  const columns = [
    {
      ...
      if(check_presence.length > 0){
        ...
        values[`area_Process${index}`] = value1[0].area_Process
        ...
        function onFinish(values){
          let datason = data_back.map((item)=>{
            if(item.key == find_key){
              ...
              forchange.area_Process = values[`area_Process${index}`]
        ...
        function onFinishFailed(){
          ...
          layout="inline">
                     <FormItem label={`Title`} rules={[{ required: true, message: 'Please input your Title!\'}' }]} name={`area_Process${index}`}>
    ...
    let check_presence =   foredit.filter((items)=>{    
      ...
      if(check_presence.length > 0){
        ...
        return {
          ...
          key : item.key,
          area_Process : item.area_Process,
          created : '06-09-2022',
          modified : '07-09-2022',
...
<Add_area_Process setvisible={setvisible} isvisible={isvisible} />
{/* <Setproductadd setvisible={setvisible} isvisible={isvisible} /> */}
    </div>
  );
};
    
export default Area_area_Process;
