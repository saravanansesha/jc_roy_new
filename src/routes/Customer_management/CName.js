...
// import BreadCrumb from "../../cNames/BreadCrumbs"
// import Setproductadd from "./Setproductadd"
import Add_cName from "./Add_cName"
  ...
  const columns = [
    {
      ...
      if(check_presence.length > 0){
        ...
        values[`cName${index}`] = value1[0].cName
        ...
        function onFinish(values){
          let datason = data_back.map((item)=>{
            if(item.key == find_key){
              ...
              forchange.cName = values[`cName${index}`]
        ...
        function onFinishFailed(){
          ...
          layout="inline">
                     <FormItem label={`Title`} rules={[{ required: true, message: 'Please input your Title!\'}' }]} name={`cName${index}`}>
...
<Add_cName setvisible={setvisible} isvisible={isvisible} />
